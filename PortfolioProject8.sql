...
# Purpose:	    The main purpose of the project is to use PL/SQL to generate reports and run DML operations on pumpkins sales data
#               The project starts with a bit of regular SQL to do some final cleanup and create a table for joins to be used in the
#               PL/SQL Code
# Last Update:	February 6th, 2024
# Execution		The original Data can be imported from Kaggle. Much preliminary work has been done on the data, as documented by projects
#               6 and 7 of this portfolio.
...
update uspumpkins
set dateofsale = to_date(date_sold, 'MM/DD/YY');

alter table uspumpkins
drop column datesold;

alter table uspumpkins
rename column dateofsale to date_sold;
...
-- Here is a simple PL/SQL block that prints the average lowprice of the howden variety pumpkin
...
set serveroutput on;
-- This is how you make your own row type with an associative array
...
-- We can also do this with the rowtype of our uspumpkins table with the cursor
...
-- Here we're looping through a cursor to generate a report of information about the first 99 orders
...
-- Here we use a for loop witht the cursor to generate the same report, but with the for loop we can determine how many rows
-- of the cursor we have displayed
...
-- The for in clause opens and even closes the cursor for us
...
-- We can even just use the query directly inside of the for in loop as below to generate the same output as above
...
-- Here is a cursor with parameters that allows the user to input which rows according to the origin of the pumpkin
...
-- Here the bind variables are used to bring up the orders and the code will prompt for two origins. We'll also use
-- a for loop here to loop through a print each of the sales IDs.
...
-- Now we'll use the same cursor with two parameters, one for where the pumpkins were sold, and the other where the pumpkins came from
...
-- We'll make a dummy table to do updates on with a cursor
...
-- We'll also use the for update clause. This will lock all the rows of the table
...
begin
 for r_pumps in c_pumps loop
  ...
  -- If we have a join like this, we can specify that we don't need the joined table's rows locked by adding
  -- a for update of clause (we can just reference one column of pump_dummy, as updating for even just one column
  -- will lock the entire row)
...
-- purposes. We'll make it 10 percent in the interest of contrast in the output
...
-- Below is how you'd run the above procedure in a block
...
-- Here we'll create a function that returns the average high price of pumpkin orders from any given place of origin
...
-- Now we'll use the function in a select statement to get the average high price by origin of each record
